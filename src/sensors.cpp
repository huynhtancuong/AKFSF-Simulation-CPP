...
GPSMeasurement GPSSensor::generateGPSMeasurement(double sensor_x, double sensor_y)
{
    ...
    if (gps_error_dis(m_rand_gen) < m_error_prob) {meas.x = sensor_x + 10.0;meas.y = sensor_y + 10.0;}
...
// Lidar Sensor
LidarSensor::LidarSensor():m_rand_gen(rand_gen),m_range_noise_std(0.0),m_theta_noise_std(0.0),m_max_range(90.0),m_id_enabled(true), m_error_prob(0.0){}
...
void LidarSensor::setLidarErrorProb(double prob){m_error_prob = prob;}
...
std::vector<LidarMeasurement> LidarSensor::generateLidarMeasurements(double sensor_x, double sensor_y, double sensor_yaw, const BeaconMap& map)
{
    ...
    std::uniform_real_distribution<double> lidar_error_dis(0.0,1.0);
    for (const auto& beacon : map.getBeacons())
    {
        ...
        if (beacon_range < m_max_range)
        {
            ...
            if (lidar_error_dis(m_rand_gen) < m_error_prob)
            {
                beacon_meas.id = -1;
            }
...
// IMU Sensor
IMUSensor::IMUSensor():
    m_rand_gen(rand_gen),m_accel_noise_std(0.0), m_gyro_noise_std(0.0), m_error_prob(0.0) {}
...
void IMUSensor::setErrorProb(double prob) {
    m_error_prob = prob;
}
...
IMUMeasurement IMUSensor::generateIMUMeasurement(double sensor_accel, double sensor_yaw_rate) {
    ...
    std::uniform_real_distribution<double> error_dis(0.0, 1.0);
    meas.accel = sensor_accel + accel_dis(m_rand_gen);
    meas.yaw_rate = sensor_yaw_rate + gyro_dis(m_rand_gen) + m_gyro_bias;
    if (error_dis(m_rand_gen) < m_error_prob) {
        meas.accel = 3*m_accel_noise_std;
        meas.yaw_rate = 3*m_gyro_noise_std;
    }
    return meas;
}
...
WheelsSpeedSensor::WheelsSpeedSensor():
    m_rand_gen(rand_gen),m_noise_std(0.0), m_error_prob(0.0), m_scaling_factor(1.0) {}
void WheelsSpeedSensor::reset() {
    m_rand_gen = rand_gen;
}

void WheelsSpeedSensor::setErrorProb(double prob) {
    m_error_prob = prob;
}

void WheelsSpeedSensor::setScalingFactor(double factor) {
...
WheelsSpeedMeasurement WheelsSpeedSensor::generateWheelsSpeedMeasurement(double right_wheel_vel, double left_wheel_vel, double base_wheel_distance) {
    ...
    std::uniform_real_distribution<double> error_dis(0.0, 1.0);
    meas.right_wheel_vel = right_wheel_vel*m_scaling_factor + odometer_dis(m_rand_gen);
    meas.left_wheel_vel = left_wheel_vel*m_scaling_factor - odometer_dis(m_rand_gen);
    meas.base_wheel_distance = base_wheel_distance;
    if (error_dis(m_rand_gen) < m_error_prob) {
        meas.right_wheel_vel = 1.5*right_wheel_vel;
        meas.left_wheel_vel = 1.5*left_wheel_vel;
    }
    return meas;
}
...
CompassSensor::CompassSensor():
    m_rand_gen(rand_gen),m_noise_std(0.0), m_error_prob(0.0), m_bias(0.0) {}
void CompassSensor::reset() {
    m_rand_gen = rand_gen;
}
void CompassSensor::setBias(double bias) {
    m_bias = bias;
}
void CompassSensor::setErrorProb(double prob) {
    m_error_prob = prob;
}
...
CompassMeasurement CompassSensor::generateCompassMeasurement(double psi) {
    ...
    std::uniform_real_distribution<double> error_dis(0.0, 1.0);
    meas.theta = psi + compass_dis(m_rand_gen) + m_bias;
    if (error_dis(m_rand_gen) < m_error_prob) {
        meas.theta = psi + M_PI;
    }
    return meas;
